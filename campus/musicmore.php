...
<?php include "common/table.php";?>
    	...
    	<div id="movieTitle">
            ...
            <p style="color:#FFFFFF; font-size:15px;">原创配乐区 Music
            &nbsp;&nbsp;
...
<!--movieDetail Start-->        
 ...
 <?php 
	...
	?>
        ...
        <img src="img/music.jpg" width="180px";/>
        </div>
	...
	$page_num =12;//每页记录数为12
        if (!isset($_GET['page_no']))//page_no 空
          ...
          $sql = "SELECT * from `material` where `material_type_id`=2 limit $start_num, $page_num";
			//$sql="SELECT * FROM  `material`  where `material_type_id`=2";
           ...
           <div style="width:200px; height:160px; margin:10px 20px 5px 22px; float:left; border-radius: 5px;">    
                <div style="margin:0px auto;">
                    <a href="musicdetail.php?id=<?php echo $row['id']?>">
                    <?php 
			/*是否存在id.jpg的文件
		...
		<a style="margin-left:40px;" href="campus.php">返回</a></div>
<!--页码显示-->
          <?php
		$sql1 = "SELECT * from `material` where `material_type_id`=2";
		$result1 = mysql_query($sql1);
		$numss = mysql_num_rows($result1);
		$page = ceil($numss/$page_num);
            if ($page_no >= 1) {
		    echo "&nbsp;&nbsp;<a href ='musicmore.php?page_no= 1'>首页&nbsp;&nbsp;</a>";}
		if($page_no > 1){
                    echo "&nbsp;&nbsp;<a href ='musicmore.php?page_no=".($page_no-1)."'>上一页</a>&nbsp;&nbsp;&nbsp;";
                }else{
                    echo '<span>上一页</span>&nbsp;&nbsp;&nbsp;';
                }
                ...
                if ($nums == $page_num) {
                    ...
                    echo "&nbsp;&nbsp;&nbsp;<a href ='musicmore.php?page_no=".($page_no+1)."' >下一页</a>";
		    echo "&nbsp;&nbsp;<a href ='musicmore.php?page_no= ".$page."'>尾页</a>";
                }else{
                    echo '&nbsp;&nbsp;&nbsp;<span>下一页</span>';
        ...
        ?>
           ...
           <!-- <div style="width:300px;  margin-top:90px; margin-left:0px;float:left;"> 
            <?php
            include "lib/connect.php";
			$sql="SELECT * FROM  `material`  where `material_type_id`=2";
			$result=mysql_query($sql);
			...
			while($result_row=mysql_fetch_array($result)){
             echo <<<TXTE
				<center><img src="$poster" width="170px" height="100px" /></center>		
					<p style="font-size:14px;" class="big"><a href="musicdetail.php?id={$result_row['id']}">{$result_row['title']}</a></p>
TXTE;
			}
             ?>
            </div>
